...
If you believe that most of what we care about can be expressed mathematically — and I mostly do, in the sense that probability theory is surprisingly accommodating — then a lot of oversight problems reduce to "are you asking the right question?" That's not a small residue. It's the most important thing a researcher does. But it's a residue we're *equipped* to handle, because it's the part that depends on intent, taste, and lived human experience rather than symbol pushing.
...
This is why I find formal methods so generative as a research direction, and why I care about tools like Lean beyond their role in mathematics. They give us a way to scale human oversight by shrinking what humans have to oversee, without pretending we've removed humans from the loop. We haven't. We're still the ones who decide what counts as the right question. As far as I can tell, that's the one thing machines genuinely can't do for us — because they don't have the thing that generates the question in the first place, which is a human life with human stakes.
