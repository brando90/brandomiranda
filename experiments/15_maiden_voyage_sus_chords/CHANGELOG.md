...
4. Everything cut is preserved verbatim in a Google Doc, "Maiden Voyage sus chords — the long cut": https://docs.google.com/document/d/1HeSoG6DYnvrnemRD0O1ZnAXnb8aeqCgwsTQAAk0gRs8/edit (private to Brando's account; not linked from the published post, since what to do with it is undecided). Nothing was rewritten or invented on the way in.
...
v0.9 — September 6, 2026 (appendix removed entirely)
------------------------------------------------------

Brando: "I think we should remove appendix...no? it looks too long and Im trying to not be repetitive."

1. The appendix is gone — both remaining sections, 569 words. The post is now body → BibTeX → Acknowledgments, 1,687 words of prose (~8.4 min), byline unchanged at "~8 min read".
   - A. The four-block map and practice plan — reference material Brando practices from, not something a reader needs mid-essay.
   - B. Every way to play over B7sus — he was right about repetition: items 2 (B Mixolydian = E major = F# Dorian), 3 (the two pentatonics) and 10 (not Lydian dominant) restate the three things the body already argues at length. The genuinely new items (hexatonic, fourths, blues, Dorian, bebop, darker colors, outside devices, rhythm) were not worth an appendix on their own.
2. The TL;DR's trailing "(The appendix adds about four minutes and is skippable.)" is gone with it. The four surviving instances of the word "appendix" in the post all refer to the appendix of Snidero's book, which is subject matter, not structure.
3. Google Doc rebuilt and re-pointed. The Drive connector can only rename a doc, not rewrite its body, so the old doc's intro ("Its appendix keeps the four-block practice map ... and the sources plus BibTeX") could not be corrected in place. New doc created with an accurate intro, and it now also carries the sources list and flags sections 3 and 6 as existing nowhere else; the old doc was moved to Drive trash (recoverable). New URL is in this file and in README.md. Old id 1ySI6iOZc4tVGSuePP5EILTYdshyR8fA0skPCuNvDme0, new id 1HeSoG6DYnvrnemRD0O1ZnAXnb8aeqCgwsTQAAk0gRs8.
4. New HARD rule in CLAUDE.md and AGENTS.md, "No appendix by default", naming both failure modes Brando identified: an appendix makes a tight post look sprawling, and in practice it restates the body. The sibling "No sources / materials / process apparatus" rule was edited in the same pass so the two agree on how a post may end — body, BibTeX, Acknowledgments.
