# Aristotle Harmonic Grant

Scratch space for drafting the Aristotle/Harmonic grant for the PI-track application.

## Status

- Created on May 15, 2026 after a repo search found no existing Aristotle/Harmonic PI-track grant proposal draft.
- The relevant existing Harmonic material appears to be CV/application tailoring, not a grant proposal.
- Draft PI-track application created in `draft_application_pi_track.md`.
- Paste-ready Typeform responses drafted in `field_responses_pi_track.md`.
- Reference Sanmi CV PDF copied into this folder; it may be stale, so prefer `sanmi_cv_context_from_user.md` for the newer pasted context until Sanmi provides the official current CV.

## Search Notes

- Current tree search terms: `aristotle`, `harmonic`, `pi track`, `pi-track`, `principal investigator`, `grant`.
- Relevant prior Harmonic commits found in history:
  - `1aa3e6c` - reordered CV coursework for a Harmonic fellowship / Aristotle-focused reviewer.
  - `5602319` - Harmonic-tailored research experience in the long and short CVs.
  - `3bbadf7` - older branch commit expanding CVs for a Harmonic Grant.

## Useful Source Material

- `stanford-ai-for-lean.md`
- `exclude/stanford-ai-for-lean-copy-blocks.md`
- `professional_documents/cvs/cv_long.tex`
- `professional_documents/cvs/cv_short.tex`
- `professional_documents/cvs/example_cvs/Koyejo_CV.pdf`
- `https://aristotle.harmonic.fun/sponsorships`
- `https://www.stanfordlean.club/`
- `https://github.com/orgs/StanfordLeanClub/repositories`
- `https://github.com/brando90/veribench-dt`
- `https://github.com/brando90/veribench-deps/tree/e3d26b4d8f5e88dd2a8fd2ecc290a0f0c6dd39a4`
- `https://cs.stanford.edu/~daneshva/publications/ai-coding-benchmarks-need-proofs-not-just-tests.pdf`

## Working Files

- `draft_application_pi_track.md` - editable PI-track draft for Sanmi / STAIR / Brando to curate before any submission.
- `field_responses_pi_track.md` - direct answers for the visible Typeform fields.
- `sanmi_cv_context_from_user.md` - curated notes from the newer CV text pasted by Brando.
- `sanmi_koyejo_cv_reference.pdf` - local reference copy of the existing Koyejo CV PDF; likely not the final current CV.

## Related Student-Track Application

- `../07_aristotle_harmonic_rising_mathematician/brando_rising_mathematician_application.md`

The two applications should be complementary, not duplicative:

- PI track: Sanmi / STAIR as the faculty-led research infrastructure application.
- Rising Mathematician track: Brando as the student-facing application focused on personal exploration, public learning, VeriBench, Stanford AI for Lean, and sharing AI/formal-verification workflows.
